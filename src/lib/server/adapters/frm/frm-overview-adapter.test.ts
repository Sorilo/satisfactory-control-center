...
describe("FRM overview adapter", () => {
  it("calls only reviewed read endpoints and normalizes documented fields", async () => {
    const fetcher = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (path === "/getPower") return jsonResponse([{ CircuitGroupID: 0, PowerProduction: 4200, PowerConsumed: 3100, PowerCapacity: 5000, PowerMaxConsumed: 3500, BatteryPercent: 75, BatteryCapacity: 1000, BatteryDifferential: 100, BatteryTimeEmpty: "00:00:00", BatteryTimeFull: "02:00:00", FuseTriggered: false, AssociatedCircuits: [1] }]);
    ...
    expect(snapshot.power).toMatchObject({ productionMw: 4200, consumptionMw: 3100, headroomMw: 1900, fuseTriggered: false });
