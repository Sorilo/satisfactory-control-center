...
| Generators | FRM `GET /getGenerators` (or fixed per-type generator endpoints) | No Companion generator series | Generator detail is current-only unless a later approved metric is added upstream. |
| Major consumers | FRM `GET /getPowerUsage`; the Control Center ranks the validated records server-side by `PowerInfo.PowerConsumed` | Prometheus category gauges provide category history, not per-building history | FRM does not sort the records. Bound input bytes and result count; never return raw IDs, class names, locations, or private labels without an explicit public schema decision. |
