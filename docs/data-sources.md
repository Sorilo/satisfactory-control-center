...
10. **Scrape versus collection cadence:** Companion polls every five seconds and RC.4 aligns the Prometheus scrape cadence to five seconds; historical resolution cannot exceed retained scrape points.
...
- The pinned monitoring Prometheus configuration declares `scrape_interval: 15s` and `evaluation_interval: 10s`. The application must not promise a finer historical resolution unless the selected deployment is separately verified and configured for it.
...
- The authorized `unraid-phase10` live-validation alias was not resolvable from this workspace on 2026-08-21. No direct-root or guessed-host fallback was used. The older sanitized Task 0 deployment fixtures remain historical evidence only.
